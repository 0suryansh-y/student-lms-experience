...
export const Route = createFileRoute(
    ...
    loader: async ({ deps, params }) => {
        ...
        const discussionList = await fetchAllDiscussionsByEntityId({
            data: { entityId: JSON.parse(resourceId), entityType: 'Lectures', page: page },
        })
        const rowsCount = await fetchAllDiscussionsCountByEntityId({
            data: { entityId: JSON.parse(resourceId), entityType: 'Lectures' }
        })
...
function RouteComponent() {
    const { courseId, resourceId } = Route.useParams()
        ...
        <div className="bg-white border rounded-xl p-6">
            <div className="space-y-6">
                <div className="flex items-center justify-end">
                    <Link to='/courses/$courseId/resources/$resourceId/discussions/create' params={{ courseId, resourceId }}>
